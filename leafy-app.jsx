...
.portrait-nick{font-size:8px;font-weight:500;letter-spacing:2px;text-transform:uppercase;color:var(--clay-light);margin-bottom:3px;text-align:center;min-height:12px}
...
.table-nick{font-size:8px;font-weight:500;letter-spacing:2px;text-transform:uppercase;color:var(--clay-light);margin-bottom:2px;min-height:12px}
...
.nick-label{font-size:10px;font-weight:500;letter-spacing:2.5px;text-transform:uppercase;color:var(--clay)}
.nick-input{font-family:'Jost',sans-serif;font-size:10px;font-weight:500;letter-spacing:2.5px;text-transform:uppercase;color:var(--clay);background:none;border:none;border-bottom:1px solid var(--clay-light);outline:none;min-width:80px}
...
.photo-prompt{background:var(--linen);border-left:2px solid var(--clay-light);padding:12px 14px;margin-bottom:12px;display:flex;align-items:center;gap:10px}
.photo-prompt-text{font-size:12px;font-weight:300;color:var(--clay);line-height:1.5;flex:1}
...
.fm-nick{font-size:8px;font-weight:500;letter-spacing:1.5px;text-transform:uppercase;color:var(--clay)}
...
.ben-plant-nick{font-size:8px;font-weight:500;letter-spacing:1.5px;text-transform:uppercase;color:var(--clay);margin-bottom:2px}
...
.tl-nick{font-size:8px;font-weight:500;letter-spacing:1.5px;text-transform:uppercase;color:var(--clay);margin-bottom:2px}
...
function NickEdit({plant,onUpdate}){
  ...
  return <div className="nick-row" onClick={()=>setEd(true)}>{plant.nickname?<span className="nick-label">{plant.nickname}</span>:<span className="nick-ghost">Name her…</span>}<Ic n="pen" size={10} stroke="var(--clay-light)" sw={1.5}/></div>;
}
  ...
  return(
    <>
      ...
      {viewMode==="table"&&filtered.length>0&&(
        <div>
          {filtered.map((p,i)=>(
            <div key={p.id} className="table-row" style={{animationDelay:`${i*.03}s`}} onClick={()=>onSelect(p)}>
              <div className="table-sil"><Sil name={p.name} size={38} fill="var(--moss-mid)"/></div>
              <div className="table-body">
                <div className="table-nick">{p.nickname||""}</div>
                <div className="table-name">{p.name}</div>
                <div className="table-bot">{p.botanical}</div>
                {UNDERRATED[p.name]&&<div className="table-fact">{UNDERRATED[p.name]}</div>}
              </div>
              <div className="table-right">
                <div className={`tpip ${p.health}`}/>
                <div className="table-hlbl">{HL[p.health]}</div>
              </div>
            </div>
          ))}
        </div>
